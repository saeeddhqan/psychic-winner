...
def smart_mut_polynomial_bounded(
	...
	"""
		...
		C by Deb that calculates the direction of mutating with bias. It roll out the directional 
...
def usual_suspects(
	...
	# The idea behind running this loop twice is: filling genes for the first tire
	# and the second for mutating genes
	for tire in range(2):
		if tire == 1:
			start = cutoff
			finish = len(population) - cutoff
			print(len(population[start:finish]), start, finish, len(population))
